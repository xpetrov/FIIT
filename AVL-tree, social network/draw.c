...
//adjust gap between left and right node
int gap = 3;  
...
asciinode * build_ascii_tree_recursive(struct node * t) 
{
  ...
  sprintf(node->label, "%s (%d)", t->user, t->quantity_of_left_children);
  //sprintf(node->label, "%d", t->total);
...
//Free all the node of the given tree
